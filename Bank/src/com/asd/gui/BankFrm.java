...
public class BankFrm extends javax.swing.JFrame {
	...
	static public void main(String args[]) {
		try {
			...
			AccountView bankView = new BankView(accountService);
			BankFrm bankFrm = new BankFrm();
			bankFrm.setAccountView(bankView);
	...
	private void createAccount(String type) {
		...
		String accountTypeSelected = accountType.equals("Ch") ? "CHECKING" : "SAVING";

		this.accountView.createAccount(customer, accountTypeSelected, accountnr, 0.0);
	}

	private Account withdraw() {
		double amount = Double.valueOf(amountDeposit);
		this.accountView.withdraw(accountnr, amount);
		Account account = this.accountView.getAccount(accountnr);
		return account;
	}

	private Account deposit() {
		double amount = Double.valueOf(amountDeposit);
		this.accountView.deposit(accountnr, amount);
		Account account = this.accountView.getAccount(accountnr);
		return account;
	}
